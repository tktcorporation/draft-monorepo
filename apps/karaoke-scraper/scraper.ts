...
async function scrapeKaraokeScores(): Promise<KaraokeScore[]> {
	...
	try {
		...
		for (const scoringType of scoringTypes) {
			...
			try {
				...
				const noDataMessage = await page
					.locator(`${resultSelector} .no_result`)
					.count();
				...
				const pageLinks = await page
					.locator(`${resultSelector} .ppage li a`)
					.all();
							...
							for (const table of tables) {
								try {
									...
									const score = scoreMatch
										? Number.parseFloat(scoreMatch[0])
										: 0;
				...
				console.log(
					`Total scores for ${scoringType.name}: ${allScores.length}`,
				);
			} catch (error) {
		...
		combinedScores.slice(0, 10).forEach((score, index) => {
			console.log(
				`${index + 1}. ${score.songName} - ${score.artist}: ${score.score} (${score.date})`,
			);
		});

		// Display statistics
		const avgScore =
			combinedScores.reduce((sum, s) => sum + s.score, 0) /
		...
		console.log(
			`Lowest score: ${combinedScores[combinedScores.length - 1].score}`,
		);
